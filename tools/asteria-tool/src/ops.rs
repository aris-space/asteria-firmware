...
use wire_types::{
    ...
    FsRemoveResp, FsStatEndpoint, FsStatReq, FsStatResp, PANIC_MSG_CAP, PanicEndpoint, PanicReq,
    PanicResp, ResetEndpoint, ResetReq, ResetResp,
};
...
pub trait RpcBackend {
    ...
    async fn fs_remove_path(&self, path: &str, expected_epoch: Option<u32>)
    -> Result<FsRemoveResp>;
    ...
    while offset < target_size {
        ...
        match resp.err {
            FsError::Ok => {
                ...
                if chunk.is_empty() {
                    bail!(
                        "rpc fs/read returned empty data before reaching snapshot size at offset {offset}"
                    );
                }
                ...
                if offset > target_size {
                    bail!(
                        "rpc fs/read returned more bytes than requested snapshot size for {path}"
                    );
                }
            }
