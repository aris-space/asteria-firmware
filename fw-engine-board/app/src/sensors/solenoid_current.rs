...
pub struct SolenoidCurrentMeasurements {
    pub fuel_main: SolenoidCurrentMeasurement,
    pub oxidizer_main: SolenoidCurrentMeasurement,
}
...
pub async fn solenoid_current_task(i2c: I2c<'static, Async, i2c::mode::Master>) -> ! {
    ...
    loop {
        let fuel_main = read_current_channel(&mut ads1015, CurrentChannel::A1).await;
        let oxidizer_main = read_current_channel(&mut ads1015, CurrentChannel::A2).await;

        match (fuel_main, oxidizer_main, last_currents) {
            (Some(fuel_main), Some(oxidizer_main), _) => {
                let currents = SolenoidCurrentMeasurements {
                    fuel_main: SolenoidCurrentMeasurement::from_sample(fuel_main),
                    oxidizer_main: SolenoidCurrentMeasurement::from_sample(oxidizer_main),
                };

                sender.send(currents);
                info!(
                    "[ADS1015] Solenoid currents: fuel main={} A, oxidizer main={} A",
                    currents.fuel_main.amps, currents.oxidizer_main.amps
                );
                last_currents = Some(currents);
            }
            (fuel_main, oxidizer_main, Some(mut currents)) => {
                if let Some(fuel_main) = fuel_main {
                    currents.fuel_main = SolenoidCurrentMeasurement::from_sample(fuel_main);
                }
                if let Some(oxidizer_main) = oxidizer_main {
                    currents.oxidizer_main = SolenoidCurrentMeasurement::from_sample(oxidizer_main);
                }
