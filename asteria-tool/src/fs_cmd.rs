...
pub async fn run_fs_action<B: RpcBackend + Sync>(
    ...
    match action {
        ...
        FsAction::Du {
            ...
            let lines =
                du_path_lines(backend, cwd, path.as_deref(), all, recursive, depth).await?;
            for line in lines {
                println!("{line}");
            }
        }
    ...
    if stats {
        print_stats_line(&format!("{label} took {:.3}s", started.elapsed().as_secs_f64()));
    }
    Ok(())
}
...
pub async fn pull_remote_to_local<B: RpcBackend + Sync>(
    ...
    match node.kind {
        ...
        FsNodeKind::Dir => {
            ...
            let (files, bytes, failures) = pull_directory_tree(backend, &remote, &local_root).await?;
...
async fn pull_directory_tree<B: RpcBackend + Sync>(
    ...
    while let Some((remote_dir, local_dir)) = stack.pop() {
        if let Err(e) = fs::create_dir_all(&local_dir) {
            push_pull_failure(
                &mut failures,
                format!("{remote_dir}: cannot create local directory {} ({e})", local_dir.display()),
            );
            continue;
        }
...
async fn pull_file_with_progress<B: RpcBackend + Sync>(
    ...
    let result = ops::pull_file_to_path_with_progress(backend, remote_path, local_path, |done, total| {
        if !is_tty {
            return;
        }
        let pct = if total == 0 {
