...
async fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Info => {
            ...
            if stats {
                print_stats_line(&format!("info took {:.3}s", started.elapsed().as_secs_f64()));
            }
        }
        Commands::Reset => {
            let (client, _connected) =
            ...
            if stats {
                print_stats_line(&format!("reset took {:.3}s", started.elapsed().as_secs_f64()));
            }
        }
        Commands::Panic { message } => {
            ...
            if stats {
                print_stats_line(&format!("panic took {:.3}s", started.elapsed().as_secs_f64()));
            }
        }
    }
