...
pub static TANK_LEVEL_WATCH: Watch<ThreadModeRawMutex, TankLevelMeasurementRaw, WATCH> =
    Watch::new();
...
#[derive(Clone, Copy, Debug)]
pub struct TankLevelMeasurementRaw {
    pub oss_tnk_lvl: u8,
}

pub struct TankLevelDriver {
    watch_handle: Sender<'static, ThreadModeRawMutex, TankLevelMeasurementRaw, WATCH>,
}

impl TankLevelDriver {
    pub fn new() -> Self {
        let watch_handle = TANK_LEVEL_WATCH.sender();

        TankLevelDriver { watch_handle }
    }
    pub fn update(&mut self, value: TankLevelMeasurementRaw) {
...
impl<'a> DigitalPressureDriver<'a> {
    ...
    pub fn update(&mut self, value: DigitalPressureMeasurementRaw) {
        ...
        let filtered_p = get_filtered_tank_p(value.oss_tnk_p1, value.oss_tnk_p2);
