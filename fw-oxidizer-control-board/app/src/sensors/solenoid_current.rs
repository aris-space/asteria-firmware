...
pub struct SolenoidCurrentMeasurements {
    ...
    pub oxidizer_vent: SolenoidCurrentMeasurement,
}
...
pub async fn solenoid_current_task(i2c: I2c<'static, Async, i2c::mode::Master>) -> ! {
    ...
    loop {
        ...
        let oxidizer_vent = read_current_channel(&mut ads1015, CurrentChannel::A2).await;

        match (dpr, oxidizer_vent, last_currents) {
            (Some(dpr), Some(oxidizer_vent), _) => {
                let currents = SolenoidCurrentMeasurements {
                    dpr: SolenoidCurrentMeasurement::from_sample(dpr),
                    oxidizer_vent: SolenoidCurrentMeasurement::from_sample(oxidizer_vent),
                };

                sender.send(currents);
                info!(
                    "[ADS1015] Solenoid currents: DPR={} A, oxidizer vent={} A",
                    currents.dpr.amps, currents.oxidizer_vent.amps
                );
                last_currents = Some(currents);
            }
            (dpr, oxidizer_vent, Some(mut currents)) => {
                if let Some(dpr) = dpr {
                    currents.dpr = SolenoidCurrentMeasurement::from_sample(dpr);
                }
                if let Some(oxidizer_vent) = oxidizer_vent {
                    currents.oxidizer_vent = SolenoidCurrentMeasurement::from_sample(oxidizer_vent);
                }
